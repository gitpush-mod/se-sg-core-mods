...
- Issues labelled `bug` or `suggestion` → `🐛-mod-feedback` (auto) **if this bundle is in the tag map**. Check `bot/src/config.js` — SG Core bundle isn't currently mapped, so nothing mirrors from here yet.
